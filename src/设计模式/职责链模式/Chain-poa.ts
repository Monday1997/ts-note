...
class Chain{
    ...
    after(fn:Function){
        let chain = new Chain(fn);
        this.setNextSuccessor(chain);
        return chain;
...
chain.passRequest(EOrderType.prepay500,true,3);
chain.passRequest(EOrderType.prepay200,true,2);
chain.passRequest(EOrderType.normal,true,1);
chain.passRequest(EOrderType.normal,true,0);
